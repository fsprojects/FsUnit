...
let equivalent f x = CustomMatcher<obj>(sprintf "Equivalent to %A" x, fun c -> 
                try 
                    ...
                    match c with
                    | :? IEnumerable as e -> f (toCollection x) (toCollection e); true
                    | _ -> false
                with
                | _ -> false)
...
let contain x = CustomMatcher<obj>(sprintf "Contains %s" (x.ToString()), fun c -> 
                match c with
...
let containf f = CustomMatcher<obj>(sprintf "Contains %s" (f.ToString()), fun c -> 
                match c with
...
let supersetOf x = CustomMatcher<obj>(sprintf "Is superset of %A" x, fun c -> 
                Set.isSuperset (Set (unbox c)) (Set x))

let subsetOf x = CustomMatcher<obj>(sprintf "Is subset of %A" x, fun c -> 
                Set.isSubset (Set (unbox c)) (Set x))

let matchList xs = CustomMatcher<obj>(sprintf "All elements from list %s" (xs.ToString()), fun ys -> 
                match ys with
                | :? list<_> as ys' -> List.sort xs = List.sort ys'
                | :? IEnumerable as e -> e |> Seq.cast |> Seq.isEmpty && xs |> Seq.isEmpty
                | _ -> false)

let private makeOrderedMatcher description comparer = CustomMatcher<obj>(description, fun c -> 
                match c with
