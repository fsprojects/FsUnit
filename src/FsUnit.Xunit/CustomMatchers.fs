         ...
         (fun f ->
             ...
             | :? (unit -> unit) as testFunc ->
                 try
                     testFunc()
                     false
                 with ex -> if ex.GetType() = t && ex.Message = m then true else false
             | _ -> false))
...
let greaterThan(x: obj) =
    CustomMatcher<obj>(sprintf "Greater than %A" x, (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) > 0))
...
let lessThan(x: obj) =
    CustomMatcher<obj>(sprintf "Less than %A" x, (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) < 0))

let lessThanOrEqualTo(x: obj) =
    CustomMatcher<obj>
        (sprintf "Less than or equal to %A" x,
         (fun actual ->
             (unbox actual :> IComparable).CompareTo(unbox x)
             <= 0))
