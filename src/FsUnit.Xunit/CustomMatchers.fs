...
let equivalent f x =
    CustomMatcher<obj>
        (sprintf "Equivalent to %A" x,
         (fun c ->
             try
                 let toCollection o =
                     o
                     |> Seq.cast
                     |> Seq.toArray :> ICollection
                 match c with
                 | :? IEnumerable as e ->
...
let equalWithin (t: obj) (x: obj) =
    CustomMatcher<obj>
        (sprintf "%A with a tolerance of %A" x t,
         (fun a ->
         ...
         (fun f ->
             ...
             | :? (unit -> unit) as testFunc ->
                 try
                     ...
                     false
                 with ex ->
                     if ex.GetType() = t then true else false
             | _ -> false))
         ...
         (fun f ->
             ...
             | :? (unit -> unit) as testFunc ->
                 try
                     ...
                     false
                 with ex ->
                     if ex.GetType() = t && ex.Message = m then true else false
             | _ -> false))
...
let greaterThan (x: obj) =
    CustomMatcher<obj>
        (sprintf "Greater than %A" x, (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) > 0))

let greaterThanOrEqualTo (x: obj) =
    CustomMatcher<obj>
        (sprintf "Greater than or equal to %A" x,
         (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) >= 0))

let lessThan (x: obj) =
    CustomMatcher<obj>
        (sprintf "Less than %A" x, (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) < 0))

let lessThanOrEqualTo (x: obj) =
    CustomMatcher<obj>
        (sprintf "Less than or equal to %A" x, (fun actual -> (unbox actual :> IComparable).CompareTo(unbox x) <= 0))
...
let private (?) (this: 'Source) (name: string): 'Result =
    ...
    if (property = null) then raise (ArgumentException(sprintf "Property %s was not found" name, "name"))
...
let subsetOf x = CustomMatcher<obj>(sprintf "Is subset of %A" x, (fun c -> Set.isSubset (Set(unbox c)) (Set x)))

let matchList xs =
    CustomMatcher<obj>
        (sprintf "All elements from list %s" (xs.ToString()),
         (fun ys ->
             match ys with
...
let choice n = CustomMatcher<obj>(sprintf "The choice %d" n, (fun x -> (ChoiceDiscriminator(n)).check(x)))

let inRange min max =
    CustomMatcher<obj>
        (sprintf "In range from %A to %A" min max,
         (fun actual ->
